...
- **The alert view had stopped returning anything.** Its hypertension code, `38341003`, does not occur in this extract - Synthea uses `59621000` - so the comorbidity branch never fired. Its 30-day window was measured from `GETDATE()`, which had moved past the end of the data. Once fixed it returned 994 patients; after the utilisation change below, 865.
...
Two further changes are decisions about definitions rather than corrections, and one defect remains in the Power BI report:

- **Utilisation now counts unplanned care only.** Counting every encounter class, 267 of the 290 utilisation alerts fired on patients attending routine wellness and ambulatory appointments - an alert that is wrong nine times in ten gets ignored. The alert now counts emergency, inpatient and urgent care encounters, which leaves 23 utilisation alerts and 865 in total (858 from the comorbidity rule). The column is renamed `Unplanned30DayEncounters` to say what it counts.
- **The alert view no longer carries the patient's name.** It identifies patients by ID, as the briefing payload already does.
- **Known defect in the report: average length of stay by encounter class.** The chart in the screenshot above shows, for each class, the average stay across *all* encounters of patients who ever had that class, rather than across that class's own encounters - the class reaches the encounters through the patient. It reads 7.48 hours for inpatient; inpatient stays average 4.75 days. Every bar in the chart reproduces exactly under that reading. The 5.78-hour card is an average over all 678K encounters, most of them same-day visits, so it is not a length of stay in any useful sense either. The fix belongs in the `.pbix` - take the class from the same table as the stay, or relate on the encounter rather than the patient, and report inpatient stays in days. Until the report and the screenshot are updated, treat both length-of-stay visuals as wrong.
